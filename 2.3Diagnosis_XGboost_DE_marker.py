...
try:
    ...
    raise ImportError("need install xgboost：pip install xgboost | conda install -c conda-forge xgboost") from e
...
def run_xgb_nested_cv(
    ...
    device: str = "cpu",   # GPU: device="cuda"
):
    ...
    assert X.ndim == 2, "dim error"
    assert set(np.unique(y)).issubset({0, 1}), "label error"
    assert X.shape[0] == y.shape[0], "dim error"
